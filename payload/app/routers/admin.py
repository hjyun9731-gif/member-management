...
def _certificate_subject_snapshot(db: Session):
    """현재 DB 기준 자격증명번호 -> 대상자 정보를 한 번에 구성한다.

    발급이력의 linked_table/link_id가 오래되었거나 회원 화면에서 자격증명번호를
    수기로 수정한 경우에도, 현재 개인/택배/예정자/양도양수/폐업 자료를 다시 읽어
    정규화된 번호(예: 26 – 0370 -> 26-370) 기준으로 재인식한다.
    DB 데이터는 이 조회에서 수정하지 않는다.
    """
    def active_rows(model):
        q = db.query(model)
        if hasattr(model, "deleted_at"):
            q = q.filter(model.deleted_at.is_(None))
        return q.all()

    members = active_rows(models.LicenseHolder)
    candidates = active_rows(models.Candidate)
    transfers = active_rows(models.TransferLedger)
    closures = active_rows(models.Closure)

    by_id = {
        "license_holders": {r.id: r for r in members},
        "candidates": {r.id: r for r in candidates},
        "transfer_ledger": {r.id: r for r in transfers},
        "closures": {r.id: r for r in closures},
    }
    member_by_id = by_id["license_holders"]

    def row_name(table_name, row):
        if table_name == "transfer_ledger":
            return (getattr(row, "transferee", "") or getattr(row, "transferor", "") or "").strip()
        return (getattr(row, "name", "") or "").strip()

    def normalize_vehicle(v):
        return "".join(str(v or "").split()).lower()

    def upgrade_to_member(table_name, row):
        if table_name == "candidates":
            mid = getattr(row, "member_id", None)
            if mid and mid in member_by_id:
                return "license_holders", member_by_id[mid]
        elif table_name == "transfer_ledger":
            mid = getattr(row, "transferee_member_id", None) or getattr(row, "member_id", None)
            if mid and mid in member_by_id:
                return "license_holders", member_by_id[mid]
    ...
    subject_map = {}
    ordered = [
        ("license_holders", members),
        ("candidates", candidates),
        ("transfer_ledger", transfers),
        ("closures", closures),
    ]
    for table_name, rows in ordered:
        for row in rows:
            cert = crud.normalize_certificate_number(getattr(row, "certificate_number", "") or "")
            resolved_table, resolved_row = upgrade_to_member(table_name, row)
            if cert and cert not in number_map:
                number_map[cert] = (resolved_table, resolved_row)
            key = (row_name(table_name, row), normalize_vehicle(getattr(row, "vehicle_number", "")))
            if key[0] and key[1] and key not in subject_map:
                subject_map[key] = (resolved_table, resolved_row)
    ...
    return {
        ...
        "upgrade_to_member": upgrade_to_member,
    }


def _resolve_certificate_log(snapshot, log):
    """발급이력 1건을 현재 회원/예정자 자료와 연결한다.

    우선순위:
    1) 현재 각 테이블의 자격증명번호 값
    2) 기존 발급이력 linked_table/link_id
    3) 발급이력의 성명+차량번호
    이 순서라서 수기 수정 후에도 재인식하면서 박영민 26-370 같은 과거 비고/이력도 보존한다.
    """
    cert = crud.normalize_certificate_number(log.certificate_number or "") or (log.certificate_number or "")
    resolved = snapshot["number_map"].get(cert)

    if not resolved and log.linked_table and log.linked_id:
        row = snapshot["by_id"].get(log.linked_table, {}).get(log.linked_id)
        if row is not None:
            resolved = snapshot["upgrade_to_member"](log.linked_table, row)

    if not resolved:
        key = ((log.target_name or "").strip(), snapshot["normalize_vehicle"](log.vehicle_number or ""))
        if key[0] and key[1]:
            resolved = snapshot["subject_map"].get(key)

    if not resolved:
        return {
            "matched": False,
            "region": "",
            "name": log.target_name or "",
            "vehicle_number": log.vehicle_number or "",
            "approval_date": "",
            "certificate_issue_date": "",
            "management_number": "",
            "source_table": log.linked_table or "",
            "source_id": log.linked_id,
        }

    table_name, row = resolved
    name = snapshot["row_name"](table_name, row)
    if table_name == "transfer_ledger":
        approval_date = getattr(row, "approval_date", "") or ""
    else:
        approval_date = getattr(row, "approval_date", "") or ""
    ...
    return {
        ...
        "name": name or log.target_name or "",
        "vehicle_number": getattr(row, "vehicle_number", "") or log.vehicle_number or "",
        "approval_date": approval_date,
        "certificate_issue_date": getattr(row, "certificate_issue_date", "") or "",
        "management_number": getattr(row, "management_number", "") or "",
...
async def list_certificate_number_logs(search: str = None, status: str = None,
    ...
    기존 certificate_number_logs를 원본으로 사용하고, 현재 회원/예정자 자료를 읽어
    인가일자와 최신 성명/차량번호를 보강한다. 별도의 발급대장 테이블에 의존하지 않는다.
    """
    items, total = crud.list_certificate_number_logs(
        db, search=search, status=status, page=page, limit=limit, sort=sort
    )
    snapshot = _certificate_subject_snapshot(db)

    counter = db.query(models.CertificateNumberCounter).order_by(
        models.CertificateNumberCounter.year.desc()
    ).first()
    latest_year = counter.year if counter else None
    last_number = counter.last_number if counter else 0
    if not counter and items:
        try:
            latest = max(
                (crud.normalize_certificate_number(x.certificate_number or "") for x in items),
                key=lambda v: tuple(map(int, v.split("-"))) if v else (-1, -1),
            )
            if latest:
                latest_year, last_number = map(int, latest.split("-"))
        except Exception:
            pass

    out = []
    for it in items:
        current = _resolve_certificate_log(snapshot, it)
        approval_date = (current["approval_date"] or "").strip()
        if approval_date:
            approval_status = "인가완료"
        elif current["matched"]:
            approval_status = "인가대기"
        else:
            approval_status = "확인필요"
        ...
        if it.status == "used":
            ...
            issuance_status = "발급(미사용)"

        out.append({
            "certificate_number": crud.normalize_certificate_number(it.certificate_number or "") or it.certificate_number,
            ...
            "approval_date": approval_date,
            "certificate_issue_date": current["certificate_issue_date"],
            "management_number": current["management_number"],
            "region": current["region"],
            "target_name": current["name"] or it.target_name,
            "vehicle_number": current["vehicle_number"] or it.vehicle_number,
            "linked_table": current["source_table"] or it.linked_table,
            "linked_id": current["source_id"] or it.linked_id,
    ...
    return {
        ...
        "latest_certificate_number": f"{latest_year:02d}-{last_number}" if latest_year is not None and last_number else None,
    }
