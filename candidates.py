...
async def register_as_member(cid: int, body: RegisterBody,
    ...
    # 자격증명 발급대장 연동 기준점은 '예정자 입력'이 아니라 바로 이 등록 완료 시점이다.
    # 인가일자 입력 → 관리번호 확정 → 신규 회원 생성이 모두 성공한 뒤에만
    # 연결된 발급대장을 인가대기 → 인가완료로 변경한다.
    ledger_updated = 0
    try:
        from app.services.certificate_ledger_service import mark_candidate_approved
        ledger_updated = mark_candidate_approved(
            db, cid, member.id, body.approval_date, user
        )
    except Exception:
        # 회원 등록은 위에서 이미 정상 커밋된 상태다.
        # 대장 연동 오류가 회원 등록 자체를 실패 처리하지 않도록 세션만 정리한다.
        db.rollback()

    return {"ok": True, "management_number": mgmt, "member_id": member.id,
            "category": member.category,
            "transfer_ledger_created": bool(member.transfer_ledger_id),
            "certificate_ledger_updated": ledger_updated}
