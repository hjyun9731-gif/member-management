...
function renderUplusStatus(d){const box=$('#uplusStatusLines');if(!box)return;const lines=[['Local Bridge 실행됨',true],['Railway 서버 연결됨',true],['U+ 통화매니저 설치 확인됨',Boolean(d?.crm_installed)],['U+ 통화매니저 실행 중',Boolean(d?.crm_running)]];box.innerHTML=lines.map(([label,ok])=>`<div class="uplus-status-line ${ok?'ok':'bad'}">${ok?'●':'✕'} ${label}</div>`).join('');const openBtn=$('#uplusOpenCrmBtn');if(openBtn)openBtn.classList.toggle('hidden',Boolean(d?.crm_running))}
async function checkUplusBridge(showToast=false){const st=$('#uplusBridgeStatus'),box=$('#uplusSetupBox');try{const d=await uplusBridgeFetch('/health',{method:'GET'});if(st){st.textContent=`연결됨 · ${d.product||'LG U+ CRM Pro'} · Bridge ${d.version||''}`;st.className='uplus-bridge-status ok'}renderUplusStatus(d);box?.classList.toggle('hidden',Boolean(d.crm_installed));if(showToast)toast('U+ CRM Pro PC 연동이 연결되어 있습니다.');return true}catch(e){if(st){st.textContent='연결 안 됨 · 이 PC에서 U+ CRM Pro Bridge를 실행해주세요.';st.className='uplus-bridge-status bad'}renderUplusStatus(null);box?.classList.remove('hidden');if(showToast)toast('U+ CRM Pro PC 연동을 확인해주세요.');return false}}
...
async function copyUplusPayload(){const m=state.smsMember;if(!m)return;const payload=`수신번호: ${primaryPhone(m.mobile||m.phone||'')}\n\n${$('#smsText').value||''}`;try{await navigator.clipboard.writeText(payload);toast('수신번호와 문자내용을 복사했습니다.')}catch(e){toast('클립보드 복사에 실패했습니다.')}}
