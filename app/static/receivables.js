const state={view:'payment',scope:'active',closureMode:'current',arrearsOnly:false,members:[],selected:null,selectedClosure:null,detail:null,actionMember:null,page:1,pages:1,limit:50,total:0,bulkBatchId:null,bulkRows:[],manualRowId:null,smsMember:null,dashboard:null};
...
function renderMonthlyAnalysis(a){
  ...
  set('perfNetPct',c.net_change_pct===null||c.net_change_pct===undefined?'비교 불가':`${Number(c.net_change_pct)>0?'+':''}${Number(c.net_change_pct).toFixed(1)}%`);
  const netCard=$('#perfNetCard');if(netCard){netCard.classList.toggle('good',Number(c.net_change||0)<0);netCard.classList.toggle('bad',Number(c.net_change||0)>0)}
