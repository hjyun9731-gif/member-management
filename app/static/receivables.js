...
function splitPhones(raw){
  ...
  if(!text)return [];
  const hits=text.match(/01[016789](?:[-\s]?\d){7,8}/g)||[];
  const out=[];
  for(const hit of hits){
    const digits=hit.replace(/\D/g,'');
    if(digits.length!==10&&digits.length!==11)continue;
    const formatted=digits.length===11?`${digits.slice(0,3)}-${digits.slice(3,7)}-${digits.slice(7)}`:`${digits.slice(0,3)}-${digits.slice(3,6)}-${digits.slice(6)}`;
    if(!out.includes(formatted))out.push(formatted);
  }
  if(!out.length){
    const digits=text.replace(/\D/g,'');
    if((digits.length===10||digits.length===11)&&digits.startsWith('01')){
      out.push(digits.length===11?`${digits.slice(0,3)}-${digits.slice(3,7)}-${digits.slice(7)}`:`${digits.slice(0,3)}-${digits.slice(3,6)}-${digits.slice(6)}`);
    }
  }
  return out;
}
function primaryPhone(raw){return splitPhones(raw)[0]||''}
function phoneCellHtml(raw){
  const phones=splitPhones(raw);
  if(!phones.length)return '<span class="phone-values"><span class="phone-number">-</span></span>';
  return `<span class="phone-values${phones.length>1?' multi':''}">${phones.map(p=>`<span class="phone-number">${esc(p)}</span>`).join('')}</span>`;
}
function phoneLineHtml(raw,sms=''){
  const count=splitPhones(raw).length;
  return `<div class="phone-line${count>1?' multi-phone':''}">${phoneCellHtml(raw)}${sms}</div>`;
}
...
function syncPerformanceView(){
  ...
  // 미수금 대시보드는 탭으로 숨기지 않는다. 한 페이지를 아래로 스크롤하며 전체 섹션을 순서대로 본다.
  $$('[data-perf-panel]').forEach(panel=>panel.classList.remove('hidden'));
  const ws=document.querySelector('.workspace');
  if(ws)ws.classList.remove('performance-workspace-hidden');
  const memberHead=$('#arrearsMemberSectionHead');
...
function renderMembers(){
  ...
  for(const m of state.members){
    ...
    if(state.view==='closed'){
      const phoneRaw=m.mobile||m.phone||'';
      const sms=m.member_id&&primaryPhone(phoneRaw)?`<button type="button" class="row-sms-btn" data-sms-id="${m.member_id}">U+ 문자</button>`:'';
      ...
      tr.innerHTML=`<td class="col-closure-no"><b>${esc(m.closure_management_number||'-')}</b></td><td class="col-type"><span class="closure-type-tag">${esc(m.closure_type||'-')}</span></td><td class="col-member"><div class="closure-person"><b>${esc(m.name||'-')}</b></div></td><td class="col-region"><span class="region-text">${esc(m.region||'-')}</span></td><td class="col-vehicle"><span class="vehicle-text">${esc(m.vehicle_number||'-')}</span></td><td class="col-phone">${phoneLineHtml(phoneRaw,sms)}</td>${stateCell}<td class="num col-balance ${ms.cls}">${ms.label==='선납금'?'선납 ':''}${ms.text}</td>`;
      ...
      const sms=primaryPhone(phoneRaw)?`<button type="button" class="row-sms-btn" data-sms-id="${m.member_id}">U+ 문자</button>`:'';
      ...
      tr.innerHTML=`<td class="col-member"><div class="member-main">${esc(m.name)}</div><div class="member-sub">${esc(m.management_number||'')}</div></td><td class="col-account"><span class="${acctClass(m.account_type)}">${esc(m.account_type)}</span></td><td class="col-vehicle"><span class="vehicle-text">${esc(m.vehicle_number||'-')}</span></td><td class="col-region"><span class="region-text">${esc(m.region||'-')}</span></td><td class="col-phone">${phoneLineHtml(phoneRaw,sms)}</td><td class="col-first-charge"><span class="first-charge-text">${esc(firstCharge)}</span></td><td class="num col-balance ${ms.cls}">${ms.label==='선납금'?'선납 ':''}${ms.text}</td><td class="col-billing"><span class="${billClass(m.billing_state)}">${esc(m.billing_state)}</span></td>${contact}<td class="manage-cell col-manage">${m.active?manage:'<span class="closed-mini">처리완료</span>'}</td>`;
...
function kstDueDateText(){const parts=new Intl.DateTimeFormat('en-US',{timeZone:'Asia/Seoul',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date());const g=t=>Number(parts.find(p=>p.type===t).value);const kstNow=new Date(Date.UTC(g('year'),g('month')-1,g('day')));kstNow.setUTCDate(kstNow.getUTCDate()+3);return `${kstNow.getUTCFullYear()}년 ${String(kstNow.getUTCMonth()+1).padStart(2,'0')}월 ${String(kstNow.getUTCDate()).padStart(2,'0')}일`}
function defaultSmsText(m){const balance=Number(m.balance||0);if(balance<=0)return `[강원도 개인소형화물협회]\n${m.name}님, 현재 ${m.account_type||'회비'} 관련 미수금 내역이 없습니다.\n\n확인 부탁드립니다.`;const amountText=`${balance.toLocaleString()}원`;return `[강원도 개인소형화물협회]\n${m.name}님, 현재 ${m.account_type||'협회비'} 미수금 ${amountText}이 확인되어 안내드립니다.\n\n계좌안내 : 농협 281-01-137254\n납부기한 : ${kstDueDateText()}까지\n\n확인 부탁드립니다.`}
function renderUplusStatus(d){
  const st=$('#uplusBridgeStatus'),openBtn=$('#uplusOpenCrmBtn');
  if(!st)return false;
  const ready=Boolean(d?.can_prepare_sms ?? (d?.crm_installed&&d?.crm_running&&d?.remote_debug_ready));
  if(ready){
    st.textContent='● U+ 연동 정상';
    st.className='uplus-bridge-status ok';
  }else if(d?.crm_installed){
    st.textContent='● U+ 연결 준비 필요';
    st.className='uplus-bridge-status bad';
  }else{
    st.textContent='● U+ 연동 프로그램 확인 필요';
    st.className='uplus-bridge-status bad';
  }
  if(openBtn)openBtn.classList.toggle('hidden',ready||!d?.crm_installed);
  return ready;
}
async function checkUplusBridge(showToast=false){
  const box=$('#uplusSetupBox');
  try{
    const d=await uplusBridgeFetch('/health',{method:'GET'});
    const ready=renderUplusStatus(d);
    box?.classList.toggle('hidden',Boolean(d.crm_installed));
    if(showToast)toast(ready?'U+ 연동이 정상입니다.':'U+ 통화매니저 연결 상태를 확인해주세요.');
    return ready;
  }catch(e){
    const st=$('#uplusBridgeStatus');
    if(st){st.textContent='● U+ 연동 확인 필요';st.className='uplus-bridge-status bad'}
    const openBtn=$('#uplusOpenCrmBtn');
    if(openBtn)openBtn.classList.add('hidden');
    box?.classList.remove('hidden');
    if(showToast)toast('U+ PC 연동을 확인해주세요.');
    return false;
  }
}
async function openCrmProNow(){try{await uplusBridgeFetch('/open-crm',{method:'POST'});toast('U+ 통화매니저 실행을 요청했습니다.');setTimeout(()=>checkUplusBridge(false),1200)}catch(e){toast('U+ 통화매니저를 실행하지 못했습니다. Bridge 연결을 확인해주세요.')}}
function openSmsComposer(m){state.smsMember=m;const phones=splitPhones(m.mobile||m.phone||'');const label=phones.length?`${phones[0]}${phones.length>1?` 외 ${phones.length-1}개`:''}`:'번호없음';$('#smsRecipient').textContent=`${m.name||''} · ${label}`;$('#smsText').value=defaultSmsText(m);$('#smsOverlay').classList.remove('hidden');$('#smsOverlay').setAttribute('aria-hidden','false');checkUplusBridge(false)}
function closeSmsComposer(){state.smsMember=null;$('#smsOverlay').classList.add('hidden');$('#smsOverlay').setAttribute('aria-hidden','true')}
async function copyUplusPhone(){
  const m=state.smsMember;if(!m)return;
  const phone=primaryPhone(m.mobile||m.phone||'');
  if(!phone)return toast('복사할 핸드폰번호가 없습니다.');
  try{await navigator.clipboard.writeText(phone);toast('핸드폰번호를 복사했습니다.')}catch(e){toast('번호 복사에 실패했습니다.')}
}
async function copyUplusText(){
  const text=$('#smsText')?.value||'';
  if(!text)return toast('복사할 문자내용이 없습니다.');
  try{await navigator.clipboard.writeText(text);toast('문자내용을 복사했습니다.')}catch(e){toast('내용 복사에 실패했습니다.')}
}
async function sendSmsNow(){const m=state.smsMember;if(!m)return;const phone=primaryPhone(m.mobile||m.phone||'').replace(/\D/g,'');const text=($('#smsText').value||'').trim();if(!phone)return toast('핸드폰번호가 없습니다.');if(!text)return toast('문자 내용을 입력해주세요.');
  // NOTE: auto_send is intentionally always false. The Bridge/CRM Pro also ignores this flag server-side —
  // the actual "지금 전송" click must always be done by the user inside U+ 통화매니저.
  try{const rr=await uplusBridgeFetch('/send',{method:'POST',body:JSON.stringify({phone,name:m.name||'',message:text,auto_send:false})});if(rr.prepared){await api(`/api/receivables/members/${m.member_id}/contacts`,{method:'POST',body:JSON.stringify({contact_date:today(),contact_method:'문자',status:'문자준비',memo:`U+ CRM Pro 문자창 입력: ${text.slice(0,120)}`})}).catch(()=>null);toast('U+ 통화매니저 문자창에 입력했습니다.\nU+ 통화매니저에서 지금 전송 버튼을 눌러주세요.');closeSmsComposer();await loadMembers(false);return}throw new Error(rr.message||'CRM Pro 자동입력을 완료하지 못했습니다.')}catch(e){checkUplusBridge(false).catch(()=>null);toast(e.name==='AbortError'?'U+ PC 연동 프로그램에 연결할 수 없습니다.':(e.message||'U+ 문자 연동에 실패했습니다.'))}}

function bind(){const compareBtn=$('#applyMonthCompare');if(compareBtn)compareBtn.onclick=()=>loadMonthComparison().catch(e=>toast(e.message));const monthA=$('#compareMonthA'),monthB=$('#compareMonthB');if(monthA)monthA.onchange=()=>{if(monthB&&monthA.value>monthB.value)monthB.value=monthA.value};if(monthB)monthB.onchange=()=>{if(monthA&&monthA.value>monthB.value)monthA.value=monthB.value};const smsClose=$('#smsCloseBtn');if(smsClose)smsClose.onclick=closeSmsComposer;const smsCancel=$('#smsCancelBtn');if(smsCancel)smsCancel.onclick=closeSmsComposer;const smsSend=$('#smsSendBtn');if(smsSend)smsSend.onclick=()=>sendSmsNow();const uplusCheck=$('#uplusCheckBtn');if(uplusCheck)uplusCheck.onclick=()=>checkUplusBridge(true);const uplusCopyPhone=$('#uplusCopyPhoneBtn');if(uplusCopyPhone)uplusCopyPhone.onclick=copyUplusPhone;const uplusCopyText=$('#uplusCopyTextBtn');if(uplusCopyText)uplusCopyText.onclick=copyUplusText;const uplusOpenCrm=$('#uplusOpenCrmBtn');if(uplusOpenCrm)uplusOpenCrm.onclick=()=>openCrmProNow();const smsOverlay=$('#smsOverlay');if(smsOverlay)smsOverlay.onclick=e=>{if(e.target===smsOverlay)closeSmsComposer()};const detailSms=$('#detailSmsBtn');if(detailSms)detailSms.onclick=()=>{if(state.detail?.member)openSmsComposer(state.detail.member)};const editBalance=$('#editBalanceBtn');if(editBalance)editBalance.onclick=openBalanceEdit;const closeBalance=$('#closeBalanceEditBtn');if(closeBalance)closeBalance.onclick=closeBalanceEdit;const cancelBalance=$('#cancelBalanceEditBtn');if(cancelBalance)cancelBalance.onclick=closeBalanceEdit;const saveBalance=$('#saveBalanceEditBtn');if(saveBalance)saveBalance.onclick=()=>saveBalanceEdit().catch(e=>toast(e.message));const balanceType=$('#balanceEditType');if(balanceType)balanceType.onchange=syncBalanceEditType;const balanceModal=$('#balanceEditModal');if(balanceModal)balanceModal.onclick=e=>{if(e.target===balanceModal)closeBalanceEdit()};const bulkBtn=$('#bulkImportBtn');if(bulkBtn)bulkBtn.onclick=openBulkImport;$('#closeBulkImportBtn').onclick=closeBulkImport;$('#bulkImportModal').onclick=e=>{if(e.target.id==='bulkImportModal')closeBulkImport()};$('#bulkPreviewBtn').onclick=()=>previewBulkImport().catch(e=>toast(e.message));$('#bulkPostBtn').onclick=()=>postBulkImport().catch(e=>toast(e.message));$('#closeManualMatchBtn').onclick=()=>{$('#manualMatchPanel').classList.add('hidden');state.manualRowId=null};$('#manualMatchSearchBtn').onclick=()=>searchManualMembers().catch(e=>toast(e.message));$('#manualMatchSearch').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();searchManualMembers().catch(err=>toast(err.message))}};const excelBtn=$('#excelExportBtn');if(excelBtn)excelBtn.onclick=()=>downloadFilteredExcel().catch(e=>toast(e.message));$$('[data-closure-mode]').forEach(b=>b.onclick=async()=>{$$('[data-closure-mode]').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.closureMode=b.dataset.closureMode;clearDetail();await loadMembers(true)});$$('.tab').forEach(b=>b.onclick=async()=>{$$('.tab').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.view=b.dataset.view;if(state.view==='closed'){state.closureMode='current';$$('[data-closure-mode]').forEach(x=>x.classList.toggle('active',x.dataset.closureMode==='current'))}clearDetail();await loadMembers(true)});let timer;$('#searchInput').oninput=()=>{clearTimeout(timer);timer=setTimeout(()=>loadMembers(true).catch(e=>toast(e.message)),120)};['regionFilter','accountFilter','contactFilter','billingFilter'].forEach(id=>$('#'+id).onchange=()=>loadMembers(true).catch(e=>toast(e.message)));$('#prevPageBtn').onclick=()=>{if(state.page>1){state.page--;loadMembers(false).catch(e=>toast(e.message))}};$('#nextPageBtn').onclick=()=>{if(state.page<state.pages){state.page++;loadMembers(false).catch(e=>toast(e.message))}};$$('.quick-amounts [data-amount]').forEach(b=>b.onclick=()=>$('#paymentAmount').value=b.dataset.amount);$('#fullAmountBtn').onclick=()=>{if(state.detail)$('#paymentAmount').value=Math.max(0,state.detail.member.balance)};$('#savePaymentBtn').onclick=()=>savePayment().catch(e=>toast(e.message));$('#saveContactBtn').onclick=()=>saveContact().catch(e=>toast(e.message));$('#openMemberWorkBtn').onclick=()=>{if(state.detail?.member)openMemberAction(state.detail.member).catch(e=>toast(e.message))};$('#memberActionType').onchange=()=>refreshActionNumber().catch(e=>toast(e.message));$('#closeActionModalBtn').onclick=closeMemberActionModal;$('#cancelActionBtn').onclick=closeMemberActionModal;$('#memberActionModal').onclick=e=>{if(e.target.id==='memberActionModal')closeMemberActionModal()};$('#saveMemberActionBtn').onclick=()=>saveMemberAction().catch(e=>toast(e.message));$('#syncBtn').onclick=async()=>{const d=await api('/api/receivables/sync');toast(`DB 동기화 완료 · 신규 ${d.profiles_created} / 부과 ${d.charges_created} / 잘못된부과정리 ${d.charges_removed||0} · Excel 재반영 없음`);await loadMembers(false);await loadSummary()};$('#kpiPendingCard').onclick=()=>{$('.tab[data-view="payment"]').click();$('#billingFilter').value='pending';loadMembers(true).catch(e=>toast(e.message))};$('#kpiPrepaidCard').onclick=()=>{$('.tab[data-view="payment"]').click();$('#billingFilter').value='prepaid';loadMembers(true).catch(e=>toast(e.message))};$('#logoutBtn').onclick=()=>{['access_token','token','authToken'].forEach(k=>localStorage.removeItem(k));location.href='/login'};$('#loginForm').onsubmit=async e=>{e.preventDefault();const body=new URLSearchParams({username:$('#loginUsername').value,password:$('#loginPassword').value});try{const r=await fetch('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});const d=await r.json();if(!r.ok)throw new Error(d.detail||'로그인 실패');saveToken(d.access_token);$('#loginOverlay').classList.add('hidden');await init()}catch(err){$('#loginError').textContent=err.message}}}
