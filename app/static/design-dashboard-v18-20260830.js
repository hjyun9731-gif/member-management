/* ================================================================
   V18 DASHBOARD NAV + REGION COMBO + FORM SPACING 2026-08-30
...
(() => {
  ...
  function visibleTextTarget(labels) {
    const wanted = labels.map(norm);
    const nodes = [...document.querySelectorAll('#subBar button, #subBar a, .sub-bar button, .sub-bar a, #content button, #content a')];
  ...
  function waitForSubTab(labels, timeoutMs = 3500) {
    const started = Date.now();
    const pick = () => {
      const target = visibleTextTarget(labels);
      if (!target) return false;
      try { target.click(); }
      catch (_) { target.dispatchEvent(new MouseEvent('click', {bubbles:true, cancelable:true, view:window})); }
      return true;
  ...
  function goRecentHistory() {
    if (!clickTopCategory('reports', '보고/집계')) return;
    waitForSubTab(['업무이력','최근업무','최근 업무','변경이력','처리이력','업무기록']);
  }
  ...
  function enhanceRegionCombo(form) {
    ...
    const listId = 'ux-region-options';
    ...
    input.setAttribute('placeholder', '입력 또는 선택');
    input.value = select.value || '';
    input.setAttribute('aria-label', '지역 입력 또는 선택');
