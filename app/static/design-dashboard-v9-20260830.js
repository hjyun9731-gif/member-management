/* ================================================================
   V9 REFERENCE UI DECORATOR 2026-08-30
...
(() => {
  ...
  async function loadDeadline() {
    ...
    el.innerHTML = `
      ...
      <div class="ux-dead-head"><span>업무</span><span>대상</span><span>기한</span><span>남은일</span></div>
  ...
  function cleanCandidatePresentation() {
    const form = $('#candForm');
    if (!form) return;
    form.querySelectorAll('span').forEach((sp) => {
      if ((sp.textContent || '').replace(/\s+/g,'').includes('없으면미가입')) {
        sp.hidden = true;
