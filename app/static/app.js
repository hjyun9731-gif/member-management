...
const CATS = {
  ...
  deadlines: {label:'기한관리',    tabs:[{id:'deadlines',label:'기한관리'}]},
...
function ensurePermitLedgerLikeNewStyle(){
  if(document.getElementById('permit-ledger-like-new-v1')) return;
  const st=document.createElement('style');
  st.id='permit-ledger-like-new-v1';
  st.textContent=`
    .permit-ledger-like-new .tbl-wrap table{font-size:12px}
    .permit-ledger-like-new .tbl-wrap th{color:#6a7181;font-weight:700;background:#fbfcfe}
    .permit-ledger-like-new .tbl-wrap td{color:#41495a;vertical-align:middle}
    .permit-ledger-like-new .tbl-wrap tbody tr:hover{background:#fafbff}
    .permit-ledger-like-new .tbl-link{color:#5e69cf!important;font-weight:700;text-decoration:none}
    .permit-ledger-like-new .tbl-link:hover{color:#4853b4!important;text-decoration:underline}
    .permit-ledger-like-new .ledger-mgmt{color:#5f58bd;font-weight:800}
    .permit-ledger-like-new .ledger-date{font-weight:700;color:#343b4c}
    .permit-ledger-like-new .ledger-muted{color:#949baa;font-size:10.5px}
    .permit-ledger-like-new .ledger-member-name{font-weight:800;color:#5964c8!important}
    .permit-ledger-like-new .ledger-joined-date{font-size:10px;color:#8b92a2;margin-left:5px;white-space:nowrap}
    .permit-ledger-like-new .ledger-expand-btn{min-width:25px;padding:2px 6px!important;border-radius:7px!important;color:#72798a!important;background:#f7f8fb!important;border-color:#e1e4eb!important}
    .permit-ledger-like-new .ledger-expand-btn:hover{background:#eef0f6!important;color:#555d70!important}
    .permit-ledger-like-new .tl-transferor-detail td{background:#fbfcfe;border-top:0!important;padding:0!important}
    .permit-ledger-like-new .tl-transferor-detail-box{display:flex;align-items:center;gap:18px;min-height:42px;padding:7px 18px 8px 55px;border-top:1px dashed #e4e7ed;color:#687083}
    .permit-ledger-like-new .tl-transferor-detail-box strong{color:#50586a}
    .permit-ledger-like-new .tl-transferor-detail-box .detail-label{font-size:10px;color:#9aa1ae;margin-right:5px}
    .permit-ledger-like-new .tl-transferor-detail-box .detail-person{display:flex;align-items:center;gap:6px}
    .permit-ledger-like-new .tl-transferor-detail-box .detail-note{font-size:10.5px;color:#8d94a3}
    .permit-ledger-like-new .member-focus-cell{background:#fdfcff}
    .permit-ledger-like-new .member-focus-head{background:#faf8ff!important;color:#5b52a5!important}
    #tlLedgerCard .tl-ledger-table th:nth-child(6){background:#fbfcfe!important;color:#6a7181!important}
    #tlLedgerCard .tl-ledger-table td:nth-child(6){background:transparent!important}
    #tlLedgerCard .tl-ledger-table th:nth-child(5){background:#faf8ff!important;color:#5b52a5!important}
    #tlLedgerCard .tl-ledger-table td:nth-child(5){background:#fdfcff!important}
    .permit-ledger-like-new .td-act{white-space:nowrap}
    .permit-ledger-like-new .badge.bdomestic,
...
function navigate(cat,sub){
  ...
  document.body.classList.toggle('deadline-mode',cat==='deadlines');
  ...
  ({
    ...
    dashboard:renderDashboard, 'monthly-report':renderMonthlyReport, 'monthly-report-fields':renderMonthlyReportFields,
...
async function renderTransferLedger(){
  ...
  const f=ST.fl.tl||{};
  document.getElementById('content').innerHTML=`
    <div class="card permit-ledger-like-new" id="tlLedgerCard">
      <div class="card-hd">
        ...
        <div class="flex gap-8">
          ...
          <button class="btn bo btn-sm" id="tlExpandAllBtn">양도자 전체보기</button>
      ...
      <div class="frow">
        ...
        ${dateOrderSel('tlDateF',f.date_order||f.member_sort||'mgmt_desc')}
        <input class="srch" id="tlSrch" placeholder="양수자, 양도자, 차량번호" value="${e_(f.search||'')}">
  ...
  const doSearch=async(pg=1)=>{
    const tlSort=document.getElementById('tlDateF')?.value||'mgmt_desc';
    const tlIsDate=(tlSort==='desc'||tlSort==='asc');
    ST.fl.tl={
      region:document.getElementById('tlRegF').value,
      member_sort:tlIsDate?undefined:tlSort,
      date_order:tlIsDate?tlSort:undefined,
      search:document.getElementById('tlSrch').value.trim()
    };
    ...
    tw.innerHTML=`<div class="tbl-wrap"><table class="tl-ledger-table">
      <thead><tr>${plainHeaders(hdrs).replace('<th>양수자</th>','<th class="member-focus-head">양수자</th>')}</tr></thead>
      <tbody>${d.items.map(r=>{
        const linked=!!(r.transferor_member_id&&r.transferee_member_id);
        const teeStatus=(r.transferee_membership_status||'').trim();
        const teeJoin=teeStatus==='가입'?(r.transferee_membership_date||r.membership_date||''):'';
        const torStatus=(r.transferor_membership_status||'').trim();
        const torJoin=torStatus==='가입'?(r.transferor_membership_date||''):'';
        const teeName=r.transferee_member_id
          ?`<a class="tbl-link ledger-member-name" onclick="viewMember(${r.transferee_member_id});return false">${fv(r.transferee)}</a>`
          :`<a class="tbl-link ledger-member-name" onclick="viewTransfer(${r.id});return false">${fv(r.transferee)}</a>`;
        const torName=r.transferor_member_id
        ...
        return `<tr class="tl-main-row" data-id="${r.id}">
          ...
          <td class="member-focus-cell">${teeName}</td>
          <td>${memBadge(teeStatus||'미확인')}</td>
          <td>${fv(r.mobile)}</td>
          <td><span class="ledger-date">${fv(r.receipt_date)}</span></td>
          <td>${fv(r.approval_date)}</td>
          <td>${fv(teeJoin)}</td>
          <td>${fv(r.certificate_issue_date)}</td>
          <td>${fv(r.certificate_number)}</td>
          ...
          <td class="td-act">
            ...
            ${!r.member_id?`<button class="btn bo btn-xs" onclick="registerTransferMember(${r.id})">회원상세</button>`:`<span class="badge b-teal" style="font-size:10px">등록완료</span>`}
        ...
        <tr class="tl-transferor-detail" id="tl-extra-${r.id}" hidden>
          <td colspan="16"><div class="tl-transferor-detail-box">
            ...
            <span class="detail-note">양도자는 참고정보이며 양수자 등록정보가 본문 기준입니다.</span>
          </div></td>
        </tr>`;
      }).join('')}</tbody>
    </table></div>${pgn(d,doSearch)}`;
  ...
  const expandAll=document.getElementById('tlExpandAllBtn');
...
async function renderClosures(){
  ...
  document.getElementById('content').innerHTML=`
    <div class="card permit-ledger-like-new" id="closureLedgerCard">
      ...
      <div class="frow">
        ...
        <select id="clTypF" class="fsel"><option value="">전체 구분</option>${CLOSURE_TYPES.map(t=>`<option value="${t}" ${f.closure_type===t?'selected':''}>${t}</option>`).join('')}</select>
        <select id="clDtF" class="fsel"><option value="">신규+이전</option><option value="신규자료" ${f.data_type==='신규자료'?'selected':''}>신규자료</option><option value="이전자료" ${f.data_type==='이전자료'?'selected':''}>이전자료</option></select>
  ...
  // 신규등록대장과 같은 흐름을 유지하되 폐업업무에 필요한 기존관리번호/처리정보를 보존한다.
  const hdrs=[
  ...
  const doSearch=async(pg=1)=>{
    ...
    const q=new URLSearchParams({page:pg,limit:50,...Object.fromEntries(Object.entries(ST.fl.cl).filter(([,v])=>v))});
    ...
    tw.innerHTML=`<div class="tbl-wrap"><table>
      ...
      <tbody>${d.items.map(r=>{
        ...
        return `<tr>
          ...
          <td>${fv(r.region)}</td>
          <td><a class="tbl-link" onclick="viewClosure(${r.id});return false">${fv(r.vehicle_number)}</a></td>
          <td><a class="tbl-link ledger-member-name" onclick="viewClosure(${r.id});return false">${fv(r.name)}</a></td>
...
// ===== MONTHLY REPORT =====
// 상위: 화면 전환용 컨테이너. 인쇄용 보고서(월례_업무현황_보고서.html 양식) / 상세 자동집계(기존 화면) 두 가지를 제공.
async function renderMonthlyReport(){
  ...
  document.getElementById('mrTabPrint').onclick=()=>{ST.inner['monthly-report']='print';renderMonthlyReport();};
  document.getElementById('mrTabDetail').onclick=()=>{ST.inner['monthly-report']='detail';renderMonthlyReport();};
  if(inner==='detail') renderMonthlyReportDetail(); else renderMonthlyReportPrint();
}

// ----- 인쇄용 보고서 (월례_업무현황_보고서.html 양식 그대로) -----
async function renderMonthlyReportPrint(){
  const y=ST.reportYear,m=ST.reportMonth;
  const body=document.getElementById('mrInnerBody');
  body.innerHTML=`<div class="loading-box"><div class="spin"></div><p>월례보고서 불러오는 중...</p></div>`;
  const d=await api('GET',`/api/reports/monthly-report/full?year=${y}&month=${m}`).catch(()=>null);
  if(!d){body.innerHTML=`<div class="empty-box"><div class="empty-ico">📄</div><p class="empty-txt">불러오기 실패</p></div>`;return;}

  const fields=d.fields||[];
  const byKey=Object.fromEntries(fields.map(f=>[f.key,f]));
  const val=k=>byKey[k]?byKey[k].value:'';
  const S1_SEC='1. 허가 및 회원 현황', S2_SEC='2. 당월 허가·회원 업무 처리현황',
        S4_SEC='4. 문서·증명서 등 행정업무', S5_SEC='5. 주요 특이사항 / 다음달 관리사항';
  const S2=fields.filter(f=>f.section===S2_SEC);
  const S4=fields.filter(f=>f.section===S4_SEC);
  ...
  function pairRows(list){
    const map={};
    list.forEach(f=>{
      let base=f.key,suffix=null;
      if(f.key.endsWith('_month')){base=f.key.slice(0,-6);suffix='month';}
      else if(f.key.endsWith('_cum')){base=f.key.slice(0,-4);suffix='cum';}
      if(!map[base]) map[base]={};
      if(suffix) map[base][suffix]=f; else map[base].single=f;
    });
    return Object.values(map);
  }
  function rowLabel(pair){
    const f=pair.month||pair.single||pair.cum;
    return (f.label||'').replace(/\s*(월계|\(건\)|누계)\s*$/,'').trim();
  }
  const s2rows=pairRows(S2), s4rows=pairRows(S4);

  const vt=d.vehicle_types||[];
  const ageEntries=Object.entries(d.age_groups||{});
  const maxRows=Math.max(vt.length,ageEntries.length,1);
  let s3rows='';
  for(let i=0;i<maxRows;i++){
    const v=vt[i], a=ageEntries[i];
    s3rows+=`<tr><td class="mrp-td-left">${v?e_(v.type):''}</td><td>${v?v.count:''}</td><td class="mrp-td-left">${a?e_(a[0]):''}</td><td>${a?a[1]:''}</td></tr>`;
  }
  const totalAge=ageEntries.map(([k,v])=>`${k} ${v}`).join(' / ');

  body.innerHTML=`
  <div id="mrpRoot">
    <div class="mrp-toolbar mrp-no-print">
      <button class="mrp-secondary" onclick="ST.reportMonth--;if(ST.reportMonth<1){ST.reportMonth=12;ST.reportYear--;}renderMonthlyReport()">◀ 이전 달</button>
      <span style="align-self:center;font-weight:700;color:var(--mrp-navy)">${y}년 ${m}월</span>
      <button class="mrp-secondary" onclick="ST.reportMonth++;if(ST.reportMonth>12){ST.reportMonth=1;ST.reportYear++;}renderMonthlyReport()">다음 달 ▶</button>
      <button class="mrp-secondary" onclick="dl('/api/reports/monthly/export?year=${y}&month=${m}','월례보고서_${y}_${String(m).padStart(2,'0')}.xlsx')">엑셀</button>
      <button class="mrp-secondary" onclick="saveMonthlyReportPrint(${y},${m})">저장</button>
      <button onclick="window.print()">인쇄 / PDF 저장</button>
    </div>
    <div class="mrp-sheet">
      <div class="mrp-topbar">
        <div class="mrp-org">강원도 개인소형화물협회</div>
        <div class="mrp-tag">월례보고용</div>
      </div>
      <h1>${y}년 ${m}월 월례 업무현황 보고서</h1>
      <hr class="mrp-divider">

      <div class="mrp-meta">
        <div><label>기준일</label><input id="mrpBaseDate" value="${e_(d.meta.base_date)}"></div>
        <div><label>작성자</label><input id="mrpWriter" value="${e_(d.meta.writer)}"></div>
        <div><label>확인자</label><input id="mrpChecker" value="${e_(d.meta.checker)}"></div>
      </div>
      ...
      <div class="mrp-bar">3. 차량·사업자 구성 요약</div>
      <table>
        <tr><th>차량 유형</th><th>대수</th><th>연령대</th><th>인원</th></tr>
        ${s3rows}
      </table>
      <div class="mrp-note-line"><b>전체 연령</b> &nbsp;${e_(totalAge)}</div>

      <div class="mrp-bar">4. 문서·증명서 등 행정업무</div>
      <table>
        <tr><th style="text-align:left">업무 구분</th><th>월계</th><th>누계</th></tr>
        ${s4rows.map(p=>`<tr><td class="mrp-td-left">${e_(rowLabel(p))}</td><td class="mrp-input">${p.month?cellInput(p.month):''}</td><td class="mrp-input">${p.cum?cellInput(p.cum):''}</td></tr>`).join('')}
      </table>

      ${extraSections.map(sec=>{
        const rows=fields.filter(f=>f.section===sec);
        return `<div class="mrp-bar">${e_(sec)}</div>
        <table><tr><th style="text-align:left">항목</th><th>값</th></tr>
        ${rows.map(f=>`<tr><td class="mrp-td-left">${e_(f.label)}</td><td class="mrp-input">${cellInput(f)}</td></tr>`).join('')}
        </table>`;
      }).join('')}

      <div class="mrp-bar">5. 주요 특이사항 / 다음달 관리사항</div>
      <div class="mrp-remarks"><textarea id="mrpRemarks" placeholder="내용을 입력하세요...">${e_(val('remarks'))}</textarea></div>

      <div class="mrp-footnote">※ 시스템 수치는 기준일 현재 집계값이며, 월말 확정 시 최종 수치로 교체합니다. 노란색 칸은 시스템 미연동 항목으로 직접 입력합니다.</div>
    </div>
  </div>`;
}
...
async function saveMonthlyReportPrint(y,m){
  ...
  const meta={
    writer:document.getElementById('mrpWriter')?.value||'',
    checker:document.getElementById('mrpChecker')?.value||'',
    base_date:document.getElementById('mrpBaseDate')?.value||'',
  };
  try{
...
// ----- 월례보고서 항목 관리 (관리자) -----
async function renderMonthlyReportFields(){
...
async function renderMonthlyReportDetail(){
  const y=ST.reportYear,m=ST.reportMonth;
  const target=document.getElementById('mrInnerBody');
  target.innerHTML=`<div class="loading-box"><div class="spin"></div><p>월례보고서 자동 계산 중...</p></div>`;
  const d=await api('GET',`/api/dashboard/monthly-report-auto?year=${y}&month=${m}`).catch(()=>null);
  if(!d){target.innerHTML=`<div class="empty-box"><div class="empty-ico">📄</div><p class="empty-txt">계산 실패</p></div>`;return;}
...
function _dlCleanTitle(t){
  ...
  const legacyType=String(t?.task_type||'').trim();
  if(legacyType && legacyType!==DL_DEFAULT_TYPE && v.startsWith(legacyType+' - ')){
    v=v.slice((legacyType+' - ').length).trim();
  }
  return v || t?.name || t?.vehicle_number || '기한';
}
...
function _pbdDate(s){if(!s)return new Date();const [y,m,d]=String(s).slice(0,10).split('-').map(Number);return new Date(y,m-1,d);}
...
function _pbdMoveMonth(s,n){const d=_pbdDate(s),day=d.getDate();d.setDate(1);d.setMonth(d.getMonth()+n);d.setDate(Math.min(day,new Date(d.getFullYear(),d.getMonth()+1,0).getDate()));return _pbdFmtDate(d);}
...
function _pbdState(){
  ...
  if(!ST.fl.dl||!ST.fl.dl.planbExact){
    let view=localStorage.getItem('pbdView')||'month';
    if(!['month','week','day'].includes(view)) view='month';
    ST.fl.dl={planbExact:true,view,anchor:_pbdToday(),sideOpen:true,calendarOn:true,search:'',searchOpen:false};
  }
  return ST.fl.dl;
}
function _pbdSaveState(){try{localStorage.setItem('pbdView',_pbdState().view);}catch{}}
function _pbdFiltered(items){
  const s=_pbdState();
  if(!s.calendarOn) return [];
...
function _pbdLogo(){return `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" class="pbd-logo-svg"><defs><linearGradient id="pbLogoWeb" x1="12" y1="0" x2="12" y2="24" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#777AF4"/><stop offset="0.52" stop-color="#686CF0"/><stop offset="1" stop-color="#5B64E8"/></linearGradient></defs><rect width="24" height="24" rx="5.4" fill="url(#pbLogoWeb)"/><g fill="#FFFFFF"><rect x="7.47" y="3.23" width="1.22" height="4.21" rx="0.6"/><rect x="15.31" y="3.23" width="1.22" height="4.21" rx="0.6"/><rect x="3.74" y="5.05" width="16.52" height="15.3" rx="3.1"/></g><rect x="5.17" y="9.26" width="13.66" height="9.69" rx="0.9" fill="url(#pbLogoWeb)"/><g fill="#FFFFFF"><circle cx="8.19" cy="12.35" r="1.05"/><circle cx="11.93" cy="12.35" r="1.05"/><circle cx="15.66" cy="12.35" r="1.05"/><circle cx="8.19" cy="15.86" r="1.05"/><circle cx="11.93" cy="15.86" r="1.05"/></g><circle cx="15.66" cy="15.86" r="1.05" fill="#1EDB97"/></svg>`;}
...
function _pbdHeaderTitle(){
  ...
  if(s.view==='week'){
    ...
    return a.getMonth()===b.getMonth()?`${a.getFullYear()}년 ${a.getMonth()+1}월`:`${a.getFullYear()}년 ${a.getMonth()+1}–${b.getMonth()+1}월`;
  }
  return `${d.getFullYear()}년 ${d.getMonth()+1}월 ${d.getDate()}일`;
}
function _pbdHeaderSub(){
  const s=_pbdState(),d=_pbdDate(s.anchor);
  if(s.view==='month') return '';
  if(s.view==='week'){const st=_pbdDate(_pbdWeekStart(s.anchor)),en=new Date(st);en.setDate(st.getDate()+6);return `${st.getDate()} – ${en.getDate()}일`;}
  return `${_pbdDayName(s.anchor)}요일`;
}
...
function _pbdCalendarIcon(size=17){return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"></rect><line x1="9" y1="4" x2="9" y2="20"></line></svg>`;}
function _pbdViewIcon(v){
  if(v==='month')return '<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><rect x="4.5" y="5" width="3.6" height="3.4" rx="1.1"></rect><rect x="10.2" y="5" width="3.6" height="3.4" rx="1.1"></rect><rect x="15.9" y="5" width="3.6" height="3.4" rx="1.1"></rect><rect x="4.5" y="10.5" width="3.6" height="3.4" rx="1.1"></rect><rect x="10.2" y="10.5" width="3.6" height="3.4" rx="1.1"></rect><rect x="15.9" y="10.5" width="3.6" height="3.4" rx="1.1"></rect><rect x="4.5" y="16" width="3.6" height="3.4" rx="1.1"></rect><rect x="10.2" y="16" width="3.6" height="3.4" rx="1.1"></rect><rect x="15.9" y="16" width="3.6" height="3.4" rx="1.1"></rect></svg>';
  if(v==='week')return '<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><circle cx="4" cy="7" r="1.3"></circle><circle cx="4" cy="12" r="1.3"></circle><circle cx="4" cy="17" r="1.3"></circle><rect x="8" y="5" width="5.5" height="14" rx="2"></rect><rect x="15" y="5" width="5.5" height="14" rx="2"></rect></svg>';
  return '<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><circle cx="5" cy="7" r="1.4"></circle><circle cx="5" cy="12" r="1.4"></circle><circle cx="5" cy="17" r="1.4"></circle><rect x="9" y="5" width="10" height="14" rx="2.6"></rect></svg>';
}
function _pbdSearchMatches(){return _pbdFiltered(window._pbdItems||[]).slice(0,8);}
function _pbdSearchDropHtml(){
...
window.pbdSearchBlur=()=>setTimeout(()=>{const s=_pbdState();s.searchOpen=false;_pbdRenderSearchDrop();},140);
window.pbdSearchBlurNow=()=>{const s=_pbdState();s.searchOpen=false;_pbdRenderSearchDrop();};
...
function _dlForm(r={}){
  const clean=_dlCleanTitle(r.title?r:{...r,title:''});
  const title=r.title?clean:'';
  const remRaw=r.reminder_days==null?'7,3,0':String(r.reminder_days);
