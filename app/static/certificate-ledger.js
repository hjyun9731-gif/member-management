    ...
    try {
      ...
      target.querySelectorAll('[data-cl-cancel-number]').forEach(button => {
        button.onclick = async () => {
          const num = button.dataset.clCancelNumber;
          if (!num || !confirm(`${num} 번호를 취소 처리할까요?\n번호는 재사용되지 않습니다.`)) return;
          button.disabled = true;
          try {
            await request('POST', `/api/admin/certificate-numbers/${encodeURIComponent(num)}/cancel`, {memo:'발급대장에서 대상 없는 예약번호 취소'});
            statsCache = null; statsCacheAt = 0;
            await render(state.page || 1, target);
          } catch (_) { button.disabled = false; }
        };
      });
      target.querySelectorAll('[data-cl-reactivate-number]').forEach(button => {
        button.onclick = async () => {
          const num = button.dataset.clReactivateNumber;
          if (!num || !confirm(`${num} 번호의 취소를 해제할까요?`)) return;
          button.disabled = true;
          try {
            await request('POST', `/api/admin/certificate-numbers/${encodeURIComponent(num)}/reactivate`);
            statsCache = null; statsCacheAt = 0;
            await render(state.page || 1, target);
          } catch (_) { button.disabled = false; }
        };
