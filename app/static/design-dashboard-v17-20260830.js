/* ================================================================
   V17 DASHBOARD NAV + CONSISTENT FORM DECORATOR 2026-08-30
...
(() => {
  ...
  async function loadPayment() {
    ...
    el.innerHTML = `
      ...
      <a class="ux-more ux-more-link" href="/receivables" data-ux-href="receivables">수납/미수금 관리 <span class="ux-more-arrow">→</span></a>`;
  }
  ...
  function clickTopCategory(dataCat, textFallback) {
    ...
    if (!target && textFallback) {
      target = [...document.querySelectorAll('#catNav .cat-btn, .cat-nav .cat-btn, #catNav button, .cat-nav button')]
        .find((el) => norm(el.textContent).includes(norm(textFallback)));
    }
  ...
  function waitForSubTab(labels, timeoutMs = 1800) {
    const wanted = labels.map(norm);
    const started = Date.now();
    const pick = () => {
      const candidates = [...document.querySelectorAll('#subBar .sub-tab, .sub-bar .sub-tab, #subBar button, .sub-bar button')];
      const exact = candidates.find((el) => wanted.includes(norm(el.textContent)));
      const partial = exact || candidates.find((el) => wanted.some((w) => norm(el.textContent).includes(w)));
      if (partial) {
        partial.dispatchEvent(new MouseEvent('click', {bubbles:true, cancelable:true, view:window}));
        return true;
      }
      return false;
    };
  ...
  function goMemberDashboard() {
    clickTopCategory('reports', '보고/집계');
  ...
  function goRecentHistory() {
    clickTopCategory('reports', '보고/집계');
    waitForSubTab(['업무이력','최근업무','최근 업무','변경이력','처리이력']);
  }
