...
(() => {
  ...
  async function loadDeadline() {
    ...
    const active = (list?.items || [])
      ...
      .slice(0, 4);
    const totalOpen = (s['오늘기한'] || 0) + (s['3일이내'] || 0) + (s['7일이내'] || 0) + (s['기한초과'] || 0);
    el.innerHTML = `
      <div class="ux-over-title"><span class="ux-icon mint">▦</span><strong>기한 관리</strong></div>
      <div class="ux-dead-summary">
        <div><span>진행</span><b>${fmt(totalOpen)}건</b></div>
        <div><span>7일 이내</span><b class="soon-txt">${fmt(s['7일이내'])}건</b></div>
      ...
      <div class="ux-dead-list">
        ${active.length ? active.map((x) => `
          <div class="ux-dead-row">
            <span class="ux-dead-kind">${esc(x.task_type || x.title || '기한')}</span>
            <span class="ux-dead-person">${esc(x.region ? `${x.region} ` : '')}${esc(x.name || x.vehicle_number || '-')}</span>
            <span class="ux-dead-date">${esc(x.due_date || x.start_date || '-')}</span>
            <span class="ux-dday ${ddayClass(x)}">${esc(x.dday_label || x.status || '-')}</span>
          </div>`).join('') : '<div class="ux-empty-line">진행 중인 등록 기한이 없습니다.</div>'}
      </div>
      ...
      <div class="ux-recent-list">
        ${show.length ? show.map((x) => `
          <div class="ux-recent-row">
            <span class="ux-recent-date">${esc(x.date || '-')}</span>
