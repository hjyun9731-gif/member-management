(function () {
  ...
  async function request(method, url, body) {
    ...
    const options = { method, headers: tokenHeaders(), signal: controller.signal };
    if (body !== undefined) {
    ...
    try {
      ...
      if (error && error.name === 'AbortError') throw new Error('자격증명 발급대장 서버 응답이 지연되고 있습니다. 잠시 후 다시 눌러주세요.');
      throw error;
    } finally {
      clearTimeout(timer);
    }
